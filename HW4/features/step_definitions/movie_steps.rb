...
Then /^I should see "(.*?)" before "(.*?)"$/ do |arg1, arg2|
  movies = page.all("table#movies tbody tr td[1]").map {|row| row.text}
  if movies.index("#{arg1}") > movies.index("#{arg2}")
    assert(false, "#{arg2} should be before #{arg1}")
  end
end

And /^I sort by "(.*?)"$/ do |arg1|
  click_on "#{arg1}"
end
